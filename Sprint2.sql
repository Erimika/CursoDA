...
FROM company c 
JOIN (
	SELECT t.company_id, SUM(t.amount) as Ventas, t.declined
	FROM transaction t
    WHERE t.declined = 0
    GROUP BY 1
    ) sum_ventas
ON c.id = sum_ventas.company_id
GROUP BY 1
ORDER BY 2 DESC;
...
SELECT c.company_name as Empresa, IF (COUNT(t.id) > 400,'Más de 400 transacciones','Menos de 400 transacciones') as Cant_Transacciones
FROM transaction t 
JOIN company c 
ON c.id=t.company_id
WHERE declined = 0
